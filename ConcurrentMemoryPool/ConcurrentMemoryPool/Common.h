...
// 新增：系统释放的模式
enum class SystemFreeMode {
	Release,    // 彻底归还 (MEM_RELEASE / munmap)
	Decommit    // 仅归还物理页 (MEM_DECOMMIT / madvise)，保留虚拟地址
};

// 修改 SystemFree 定义
inline static void SystemFree(void* ptr, size_t size = 0, SystemFreeMode mode = SystemFreeMode::Release)
{
#ifdef _WIN32
	if (mode == SystemFreeMode::Release) {
		// MEM_RELEASE 时 size 必须为 0
...
#else
	// Linux 下
	if (mode == SystemFreeMode::Decommit) {
		madvise(ptr, size, MADV_DONTNEED); // 需包含 <sys/mman.h>
	}
	else {
		munmap(ptr, size);
	}
